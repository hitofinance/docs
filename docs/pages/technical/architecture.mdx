...
Hito Finance's mHYPE Liquid Staking Derivatives (LSD) system is built on top of the EVM LSD Stack from StaFi's AI-powered LSaaS. The system enables users to stake their HYPE tokens and receive mHYPE tokens in return, which represent their staked position and rewards.
...
Users can stake their HYPE tokens through the `stake` method and receive equivalent mHYPE tokens in return. The amount of mHYPE tokens received is calculated based on the current rate:

```
mHYPE Amount = Stake Amount * Total mHYPE / Escrow HYPE
```

Where:
- `mHYPE Amount` is the amount of mHYPE tokens being minted to the staker
- `Stake Amount` is the amount of HYPE tokens being staked
- `Total mHYPE` is the total amount of mHYPE tokens minted and distributed to users
...
mHYPE holders can call the `unstake` method to exchange their mHYPE tokens back for HYPE tokens. The process involves:

1. Burning mHYPE tokens
...
4. Updates the exchange rate between HYPE and mHYPE

![Relay Service](/images/relay-service.png)

## Rate Management

The system maintains a dynamic exchange rate between HYPE and mHYPE tokens that:

1. Updates based on staking rewards
2. Has rate change limits to prevent volatility
3. Is tracked per era
4. Affects the amount of mHYPE tokens minted/burned
